...
public extension SFTools {
    ...
    /// 通过协议打开页面
    /// - Parameter path: 协议链接
    static func openSchemeUrl(_ path: String, completion: ((Bool) -> Void)? = nil) {
        if let urlPath = URL.init(string: path) {
            if UIApplication.shared.canOpenURL(urlPath) {
                UIApplication.shared.open(urlPath, completionHandler: completion)
            } else {
                fatalError("can not open URL path: \(path)")
            }
        } else {
            fatalError("URL path is not legal: \(path)")
        }
    }
}
